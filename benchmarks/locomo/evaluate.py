...
# Tried an inference-permission variant of this prompt after category 3
# (inferential "would X likely..." questions) came back as the worst-scoring
# category on a real full run (16.6% avg F1) with 19% of its near-zero
# answers being hedges ("cannot be determined from context"). The
# hypothesis -- that "using ONLY the provided context" reads to the model
# as "refuse to infer" -- was real, but the fix wasn't: validated on the
# actual 96 category-3 questions (same context, same model, regenerated
# fresh) it made things WORSE, not better -- 0.166 avg F1 dropped to 0.118,
# and hedge rate went UP (19% to 23%) despite explicitly telling the model
# to stop hedging. Reverted to this original prompt. See BUGS.md for the
# full before/after data; a real fix for category 3 still needs finding.
QA_SYSTEM_PROMPT = (
